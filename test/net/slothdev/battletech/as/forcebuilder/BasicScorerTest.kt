...
internal class BasicScorerTest {
    ...
    fun `difference between side1 point value and target matters`() {
        ...
        val score = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo", 0))),
                           setOf(Miniature(Color.GREEN, GameUnit("bar", 1))))
    ...
    fun `difference between side2 point value and target matters`() {
        val scorer = BasicScorer(1, 1)
        val score = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo", 1))),
                           setOf(Miniature(Color.GREEN, GameUnit("bar", 0))))
        assertTrue(score.finalScore < 1.0)
    }
    ...
    fun `no differences gives a score of 1`() {
        ...
        val score = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo", 10))),
    ...
    fun `difference between point values of the sides matters more than between side and total`() {
        val scorer = BasicScorer(10, 1)
        val bigDiffBetweenSides = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo", 14))),
                                         setOf(Miniature(Color.GREEN, GameUnit("bar", 6))))
        val smallDiffBetweenSides = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo", 6))),
                                           setOf(Miniature(Color.GREEN, GameUnit("bar", 6))))
    ...
    fun `side1 zero units yields zero score`() {
        val scorer = BasicScorer(10, 1)
        val score = scorer(setOf(), setOf(Miniature(Color.GREEN, GameUnit("foo", 14))))
    ...
    fun `side2 zero units yields zero score`() {
        val scorer = BasicScorer(10, 1)
        val score = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo", 14))), setOf())
        assertEquals(0.0, score.finalScore)
    }
    ...
    fun `side1 too few units yields zero score`() {
        ...
        val score = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo1", 14))),
                           setOf(Miniature(Color.GREEN, GameUnit("bar1", 6)),
                                 Miniature(Color.GREEN, GameUnit("bar2", 6))))
        assertEquals(0.0, score.finalScore)
    }
    ...
    fun `side2 too few units yields zero score`() {
        val scorer = BasicScorer(10, 2)
        val score = scorer(setOf(Miniature(Color.GREEN, GameUnit("foo1", 14)),
                                 Miniature(Color.GREEN, GameUnit("foo2", 14))),
                           setOf(Miniature(Color.GREEN, GameUnit("bar1", 6))))
        assertEquals(0.0, score.finalScore)
    }
