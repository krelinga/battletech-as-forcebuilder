...
internal class BuilderTest {
    @Nested inner class Build {
        ...
        @Test fun `just enough miniatures works`() {
            val minis = setOf(Miniature(Color.GREEN, GameUnit("foo", 10)),
                              Miniature(Color.GREEN, GameUnit("bar", 10)),
                              Miniature(Color.GREEN, GameUnit("baz", 10)),
                              Miniature(Color.GREEN, GameUnit("biff", 10))
                             )
        ...
        @Test fun `finds optimal solution`() {
            val foo = Miniature(Color.GREEN, GameUnit("foo", 50))
            val bar = Miniature(Color.GREEN, GameUnit("bar", 50))
            val baz = Miniature(Color.GREEN, GameUnit("baz", 51))
            val biff = Miniature(Color.GREEN, GameUnit("biff", 49))
            val targetPv = 100
        ...
        fun `supports multiple units per-miniature`() {
            ...
            val m1 = Miniature("M1", Color.GREEN, setOf(m1u1, GameUnit("M1-U2", 75)))
            val m2u1 = GameUnit("M2-U1", 50)
            val m2 = Miniature("M2", Color.GREEN, setOf(m2u1, GameUnit("M2-U2", 75)))
            val targetPv = 50
        ...
        fun `supports multiple units per-miniature last unit can be chosen`() {
            val m1u1 = GameUnit("M1-U1", 50)
            val m1 = Miniature("M1", Color.GREEN, setOf(GameUnit("M1-U2", 75), m1u1))
            val m2u1 = GameUnit("M2-U1", 50)
            val m2 = Miniature("M2", Color.GREEN, setOf(GameUnit("M2-U2", 75), m2u1))
            val targetPv = 50
        ...
        @Test fun `not enough miniatures throws exception`() {
            val b = Builder(setOf(Miniature(Color.GREEN, GameUnit("foo", 10))), BasicScorer(1, 1))
