...
internal class BuilderTest {
    @Nested inner class Build {
        ...
        @Test fun `just enough miniatures works`() {
            val minis = setOf(Miniature(Unit("foo", 10)), Miniature(Unit("bar", 10)),
                              Miniature(Unit("baz", 10)), Miniature(Unit("biff", 10))
            )
            val b = Builder(minis)
        ...
        @Test fun `finds optimal solution`() {
            val foo = Miniature(Unit("foo", 50))
            val bar = Miniature(Unit("bar", 50))
            val baz = Miniature(Unit("baz", 51))
            val biff = Miniature(Unit("biff", 49))
            val targetPv = 100
        ...
        fun `supports multiple units per-miniature last unit can be chosen`() {
            val m1u1 = Unit("M1-U1", 50)
            val m1 = Miniature("M1", setOf(Unit("M1-U2", 75), m1u1))
            val m2u1 = Unit("M2-U1", 50)
            val m2 = Miniature("M2", setOf(Unit("M2-U2", 75), m2u1))
            val targetPv = 50
            val expected = Forces(targetPv, setOf(Miniature("M1", setOf(m1u1))),
                                  setOf(Miniature("M2", setOf(m2u1))))
            val builder = Builder(setOf(m1, m2))
        ...
        @Test fun `not enough miniatures throws exception`() {
            val b = Builder(setOf(Miniature(Unit("foo", 10))))
